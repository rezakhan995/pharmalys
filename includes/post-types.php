...
function pe_setup_post_types() {
	...
	$product_args = array(
		...
		'supports'           => apply_filters( 'pe_product_supports', array( 'title', 'editor', 'thumbnail', 'revisions', 'author' ) ),
	);
	register_post_type( 'pe-product', apply_filters( 'pe_product_post_type_args', $product_args ) );

	/** Register Product Post Type */
	$story_labels = apply_filters(
		'pe_story_labels',
		array(
	...
	$story_args = array(
		...
		'show_in_menu'       => 'pharmalys-essential',
		'supports'           => apply_filters( 'pe_story_supports', array( 'title', 'editor', 'thumbnail', 'revisions', 'author' ) ),
	);
